          ...
          <div className="grid lg:grid-cols-2 grid-cols-1 gap-6 items-center">
            <div>
              <div className="z-10 relative">
                <img
                  src={creative2}
                  className="lg:h-72 h-full shadow rounded-md"
                />
              </div>
              <div className="-mt-28 xl:block hidden">
                <img
                  src={creative7}
                  className="h-72 shadow rounded-md ms-auto"
                />
              </div>
            </div>
            <div>
                ...
                {faqContent.map((faq, idx) => {
                  return (
                    <div
                      key={idx}
                      className={cn(
                        "hs-accordion border border-default-200 bg-default-50 backdrop-blur-3xl rounded-lg overflow-hidden",
                        { active: idx == faqContent.length - 1 }
                      )}
                      id={`faq-${idx}`}
                    >
                      <button
                      ...
                      >
                        <div className="px-6 pb-4 pt-0">
                          <p className="text-sm  mb-2">{faq.description}</p>
                        </div>
                      </div>
                    </div>
                  );
