...
const portfolioImages = [photography3, photography2, photography6, photography10]
const portfolioImages2 = [photography9, photography2, photography5, photography10]
const Portfolio = () => {
  return (
    ...
    <div className="container">
      <div className="flex flex-wrap items-center justify-between gap-5">
        ...
        <div>
          <Link to="" className="inline-flex items-center justify-center gap-2 border border-default-200 bg-primary px-6 py-2 text-white transition-all duration-300 hover:bg-primary-700">View All</Link>
        </div>
      </div>
    </div>
    <div className="mt-14">
      <div className="relative m-auto flex gap-8 overflow-hidden">
        <div className="marquee__group flex min-w-full flex-shrink-0 items-center justify-around gap-8">
          {portfolioImages.map((image, idx) => (
            <img key={idx} className="h-72 w-full object-cover" src={image} />
          ))}
        </div>
    ...
    <div className="mt-12 flex items-center justify-center">
      <Link to="" className="inline-flex items-center justify-center gap-2 rounded border border-default-200 px-6 py-2 text-base font-medium text-default-950 transition-all duration-300 hover:bg-primary hover:text-default-950">View More
