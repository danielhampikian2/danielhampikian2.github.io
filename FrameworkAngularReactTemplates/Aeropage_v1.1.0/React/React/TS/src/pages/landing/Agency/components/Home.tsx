...
const Home = () => {
  ...
  return (
    <section id="home" className={"bg-cover bg-no-repeat"} style={{ backgroundImage: `url(${bgLineImg})` }}>
                  ...
                  return (
                    <SwiperSlide key={idx}>
                      <div className="flex-col flex items-start justify-end xl:h-full">
                        <span className="text-base font-medium uppercase tracking-wider text-default-800">{item.title}</span>
                        <h2 className="md:text-4xl/snug text-3xl font-semibold text-default-950 mt-6">{item.name}</h2>
                        <p className="md:w-3/4 text-base  mt-5 mb-7">{item.description}</p>
                        <Link to="" className="inline-flex items-center justify-center gap-2 border border-default-200 text-default-950 py-2 px-6 rounded-md hover:bg-primary hover:text-white transition-all duration-300">Get Solution
