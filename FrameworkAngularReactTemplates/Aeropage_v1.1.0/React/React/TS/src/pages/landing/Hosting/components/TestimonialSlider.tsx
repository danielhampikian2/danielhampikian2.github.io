    ...
    <section id="testimonial">
      <div className="container">
        <div className="bg-default-100 dark:bg-default-50">
          <div className="py-20 bg-no-repeat bg-cover" style={{ backgroundImage: `url(${themeMode == "light" ? bgLine : bgLineDark})` }}>
            <div className="relative">
              <div className="max-w-2xl mx-auto text-center mb-12">
                <h2 className="md:text-4xl text-3xl font-medium text-default-950">Explore the feedback from our clients.</h2>
              </div>
              <Swiper
                  ...
                  return (
                    <SwiperSlide className="swiper-slide" key={idx}>
                      <div className="max-w-4xl mx-auto text-center">
                        <div className="p-6">
                          <img src={slide.image} className="h-14 w-14 rounded-full mx-auto" />
                          <h3 className="text-xl font-medium text-default-950 mt-5">{slide.name}</h3>
                          <p className="text-base">{slide.location}</p>
                          <p className="md:text-2xl mt-4 text-default-800">"{slide.description}"</p>
                          <div className="flex items-center justify-center gap-2 mt-4">
                            {Array.from(new Array(5)).map((_val, idx) => {
                              return (
                                <LuStar key={idx} className="h-5 w-5 stroke-yellow-300" />
                              )
                            })}
                          </div>
                        </div>
