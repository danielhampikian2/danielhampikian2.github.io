...
import { LuDownload, LuPaperclip, LuPhone, LuSend, LuTrash2, LuUsers, LuVideo, LuCircle } from "react-icons/lu"
...
const UserMessage = ({ message, toUser }: { message: ChatMessage; toUser: ChatUser }) => {
  return (
    <div className={`${message.from.id === toUser.id ? "ms-auto  flex-row-reverse  text-end" : "text-start"} group flex w-11/12 items-start gap-3`}>
      <div className="text-center">
        <img src={message.from.avatar} className="h-8 rounded-md" />
        <p className="pt-0.5 text-xs">{message.sendOn}</p>
      </div>
      {message.message.type === 'text' && (
        <div className={cn("max-w-3/4 rounded p-3", message.from.id === toUser.id ? "bg-primary" : "bg-default-100")}>
          <p className={cn("relative text-xs font-bold", message.from.id === toUser.id ? "text-white block" : "text-default-700")}>
            {message.from.name}
          </p>
          <p className={cn("pt-1", message.from.id === toUser.id && "text-white/90")}>{message.message.value}</p>
        </div>
      )}
            ...
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div className="text-start text-default-400">
                  <Link to="" className="text-sm font-bold">{message.message.value.file}</Link>
  ...
  return (
    <div className="w-full overflow-hidden rounded-xl border border-default-200 bg-white dark:bg-default-50">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 py-1.5">
          <div className="sm:w-7/12">
            <div className="flex items-center gap-2">
              <img src={selectedUser.avatar} className="me-2 h-9 rounded-full" alt="Brandon Smith" />
              <div>
                <h5 className="text-base font-medium text-default-700">
                  {selectedUser.name}
                </h5>
                <p className="mt-1.5 flex items-center text-xs text-default-400">
                  <LuCircle fill="orange" size={10} className="me-1 text-red-400" />
                  Offline
                </p>
              </div>
            </div>
      ...
      <div className="border-t border-default-200 bg-white p-6 dark:bg-default-50">
        <form name="chat-form" className="flex items-center gap-2" onSubmit={handleSubmit(sendChatMessage)}>
          <TextFormInput
            name="newMessage"
          ...
          <div className="flex w-auto gap-1">
            <Link to="" className="rounded bg-default-200 px-3 py-3 text-default-800 hover:bg-default-800/20"><LuPaperclip /></Link>
