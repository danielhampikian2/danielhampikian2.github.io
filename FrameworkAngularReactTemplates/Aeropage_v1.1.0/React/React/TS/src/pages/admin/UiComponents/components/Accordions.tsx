      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="hs-accordion-group">
          {Array.from(new Array(3)).map((_val, idx) => {
            return (
              <div key={idx} className={cn("hs-accordion", idx === 0 && "active")} >
                <button className="hs-accordion-toggle inline-flex w-full items-center gap-x-3 rounded-lg py-3 text-start font-semibold text-default-800 hover:text-default-950 disabled:pointer-events-none disabled:opacity-50 hs-accordion-active:text-primary" aria-controls="hs-basic-collapse-one">
                  <LuMinus className="hidden size-4 hs-accordion-active:block" />
                  <LuPlus className="size-4 hs-accordion-active:hidden" />
                  Accordion #{idx + 1}
                </button>
                <div className={cn("hs-accordion-content w-full overflow-hidden transition-[height] duration-300", idx != 0 && "hidden")}>
...
const CardAccordion = () => {
  const faqs = ["What is Lorem Ipsum?", " Why do we use it?", "Where does it come from?"]
  return (
            ...
            return (
              <div key={idx} className="rounded-md border border-default-200">
                <div className={cn("hs-accordion", { "active": idx === 0 })}>
                  <button className="hs-accordion-toggle group inline-flex w-full items-center gap-x-3 p-3 text-left font-semibold text-default-600 transition hover:text-default-950" aria-controls="hs-basic-with-title-and-arrow-stretched-collapse-one">
                    {faq}
                    <LuChevronDown className="ms-auto size-4 transition-all hs-accordion-active:rotate-180" />
                  </button>
                  <div className={cn("hs-accordion-content w-full overflow-hidden transition-[height] duration-300", { "hidden": idx != 0 })} aria-labelledby="hs-basic-with-title-and-arrow-stretched-heading-one">
