...
const ProjectCard = ({ project }: { project: ProjectType }) => {
  const { comment, date, description, images, progress, progressClassName, status, subTitle, task, title, text } = project
  return (
    <div className="overflow-hidden rounded-md border border-default-200 bg-white dark:bg-default-50">
      <div className="flex items-center justify-between border-b border-default-200 px-4 py-3">
        ...
        <div className={cn("rounded-md px-1.5 py-1 text-xs font-medium text-white", status === "Pending" ? "bg-amber-500" : status === "In Progress" ? "bg-primary" : "bg-teal-500")} role="alert">
          <span>{status}</span>
        </div>
      </div>
      <div className="flex flex-col">
        <div className="px-6 py-3">
          <h5 className="my-2">
            <p role='button' className="text-default-900">{subTitle}</p>
          </h5>
          ...
          <div className="flex -space-x-2">
            {images.map((image, idx) => (
              <span role='button' key={idx}>
                <img className="inline-block size-12 rounded-full border-2 border-default-50" src={image} alt="Image Description" />
              </span>
            ))}
            {text &&
              <span>
