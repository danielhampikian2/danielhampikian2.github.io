...
import { AdminLayout, AuthLayout, DefaultLAyout, LandingLayout } from '@/layouts'
    ...
    <Routes>
      ...
      <Route>
        {adminRoutes.map((route, idx) => (
          <Route
            path={route.path}
            element={isAuthenticated ? <AdminLayout {...props}>{route.element}</AdminLayout> : <Navigate to={{ pathname: '/auth/sign-in', search: 'redirectTo=' + route.path }} />}
            key={idx}
          />
        ))}
      </Route>
