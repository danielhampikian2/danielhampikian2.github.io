<script lang="ts">
	...
	import { CodesandboxIcon, DatabaseIcon, TruckIcon } from 'lucide-svelte';
	import ServiceCard3 from '$lib/components/widgets/ServiceCard3.svelte';
...
<section id="services" class="py-10 lg:py-20">
	<div class="container">
        ...
        <span
					class="py-1 px-3 rounded-md text-xs font-medium uppercase tracking-wider border border-primary bg-primary/20 text-primary">Our Services</span>
			<h2 class="md:text-4xl text-3xl font-medium text-default-950 mt-3">Comprehensive Plan</h2>
		</div>

		<div class="grid 2xl:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6">
			{#each serviceData as service}
				<ServiceCard3 {service} />
			{/each}
		</div>
	</div>
</section>