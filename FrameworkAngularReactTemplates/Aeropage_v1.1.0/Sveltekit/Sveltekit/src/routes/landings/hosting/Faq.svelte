			...
			<div class="hs-accordion-group space-y-4">
				...
				{#each faqData as faq,idx}
					<div class="relative z-20 w-full lg:w-full">
						...
						<div
							class={`hs-accordion overflow-hidden border border-default-200 bg-white transition-all duration-500 hs-accordion-active:-translate-y-2 hs-accordion-active:translate-x-2 dark:bg-default-50 ${idx===0?'active':''}`}
							id={`faq-${idx}`}>
							<button
								...
								<ChevronUpIcon
									class="h-6 w-6 stroke-default-950 transition-all duration-500 hs-accordion-active:-rotate-180" />
							</button>
							<div
