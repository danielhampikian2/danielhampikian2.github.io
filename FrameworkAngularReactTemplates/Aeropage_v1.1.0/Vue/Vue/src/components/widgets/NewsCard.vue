<template>
  <div class="relative p-10">
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-12">
      ...
      <div class="lg:col-span-6">
        <h5 class="mb-10 underline">
          <a href="#" class="text-xl font-medium text-default-950">{{ news.title }}</a>
        </h5>
        <div class="flex items-center gap-7">
          <a v-for="(tag, idx) in news.tags" :key="idx" href="#" class="text-sm uppercase tracking-widest text-default-950">{{ tag }}</a>
        </div>
      </div>

      <div v-if="news.publish" class="flex items-center lg:col-span-3 lg:justify-end">
    ...
    <div
      ...
      :style="{ backgroundImage: `url(${news.image})`, transition: 'opacity 0.4s, background-position 10s linear' }"
    >
...
import type { NewsType } from '@/types/landing'
