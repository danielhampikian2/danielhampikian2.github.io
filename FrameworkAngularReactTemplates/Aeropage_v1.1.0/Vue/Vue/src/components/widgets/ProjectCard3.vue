  ...
  <div class="overflow-hidden rounded-md border border-default-200 bg-white dark:bg-default-50">
    <div class="flex items-center justify-between border-b border-default-200 px-4 py-3">
      ...
      <div
        ...
        :class="project.status === 'in progress' ? 'bg-primary' : project.status === 'pending' ? 'bg-amber-500' : 'bg-teal-500'"
        role="alert"
      >
    ...
    <div class="flex flex-col">
      <div class="px-6 py-3">
        <h5 class="my-2">
          <router-link :to="{ name: project.link?.name }" class="text-default-900">{{ project.name }}</router-link>
        </h5>
        ...
        <div class="flex -space-x-2">
          <a v-for="(assignee, idx) in project.assignees" :key="idx" href="javascript: void(0);">
            <img class="inline-block h-12 w-12 rounded-full border-2 border-default-50" :src="assignee.image" alt="Image Description" />
          </a>
          ...
          <div class="flex items-center gap-2">
            <div class="h-1.5 w-full rounded-full bg-default-200">
              <div
                ...
                :class="project.progress >= 75 ? 'bg-primary' : project.progress < 75 && project.progress >= 50 ? 'bg-teal-500' : 'bg-amber-500/80'"
              ></div>
            </div>
