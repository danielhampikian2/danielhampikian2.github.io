<template>
  <div class="rounded-md border border-default-200 bg-white dark:bg-default-50">
    ...
    <div class="h-[440px] divide-y divide-default-200 overflow-y-auto [&::-webkit-scrollbar-track]:!bg-transparent [&::-webkit-scrollbar]:w-1">
      <div v-for="(user, idx) in userData" :key="idx" class="flex items-center p-2.5">
        <img :src="user.image" class="me-3 h-12 rounded-full" alt="shreyu" />
        <div class="flex-grow">
          <h5 class="mt-1 text-default-900">{{ user.name }}</h5>
          <h6 class="mt-1 text-default-600">{{ user.role }}</h6>
        </div>
        <div class="h-4">
          <div class="hs-dropdown relative inline-flex [--placement:left-top] rtl:[--placement:bottom-left]">
            ...
            >
              <a
                ...
                href="javascript:;"
              >
                <PencilIcon class="me-1.5 h-4 w-4" />
                <span>Edit</span>
              </a>
              <a
                class="mx-1.5 flex items-center rounded px-3 py-2 text-default-600 transition-all hover:bg-default-100 hover:text-default-700"
                href="javascript:;"
              >
                <LogOutIcon class="me-1.5 h-4 w-4" />
              ...
              <a class="mx-1.5 flex items-center rounded px-3 py-2 text-red-500 transition-all hover:bg-red-500/10" href="javascript:;">
                <Trash2Icon class="me-1.5 h-4 w-4" />
                <span>Delete</span>
              </a>
            </div>
          </div>
...
import { MoreVerticalIcon, PencilIcon, LogOutIcon, Trash2Icon } from 'lucide-vue-next'
