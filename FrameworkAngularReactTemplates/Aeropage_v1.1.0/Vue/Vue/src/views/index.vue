  ...
  <section v-for="(item, idx) in demoPages" :id="item.id" :key="item.id" class="py-20">
    <div class="mx-20">
      <div class="mx-auto text-center mb-14">
        <span class="inline-flex text-base border-x-2 border-x-primary-600 text-primary-700 font-semibold px-2 rounded-lg bg-primary/20 mb-2">{{
          item.tag
        }}</span>
      ...
      <template v-if="item.pages">
        <div class="grid lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-8 justify-content-center">
          <router-link v-for="(page, idx) in item.pages" :to="{ name: page.link.name, params: page.link.params }" :key="idx">
            <div
            ...
            >
              <div class="p-4">
                <div class="relative rounded-lg overflow-hidden">
                  ...
                  />
                  <img alt="demo-img" class="hidden dark:block w-full rounded-lg border border-default-100" :src="page.image.dark" />
                  <div
                  ...
                  >
                    ...
                    >
                      Live Preview
                      <span class="h-8 w-8 inline-flex items-center justify-center rounded-md bg-white border border-white text-primary ms-3">
                        <ExternalLinkIcon class="h-5 w-5" />
                      </span>
                    </div>
                  </div>
                </div>
                <h5 class="mt-4 text-center text-lg font-medium capitalize text-default-900">{{ page.title }}</h5>
              </div>
            </div>
          </router-link>
        </div>
      ...
      <template v-if="item.features">
          ...
          >
            <div class="p-6">
              <div class="flex flex-col justify-center items-center text-center gap-4">
                <div class="shrink">
                  <div class="inline-flex items-center justify-center h-12 w-12 bg-primary-500/20 text-primary-600 rounded-md">
        ...
        <p class="text-base text-center mt-6 font-medium text-primary-900">Empower Your Journey with Our Engaging Landing Demo.🔥</p>
      </template>
    </div>
  </section>
...
import LandingNavbar from '@/layout/partials/LandingNavbar.vue'
import LandingFooter from '@/layout/partials/LandingFooter.vue'
